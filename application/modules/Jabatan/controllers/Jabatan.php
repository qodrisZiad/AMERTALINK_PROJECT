...
class Jabatan extends CI_Controller
{
	...
	private $table = "tm_jabatan";
	private $primary_key = "fc_jabatan";
	private $secondary_key = "fv_jabatan";
	private $kolom = array("fc_jabatan","fv_jabatan","fc_status");
	...
	public function Simpan(){
		...
		$message = ""; 
			$data = array(
				'fv_jabatan' => $this->input->post('a1'),
				'fc_status' => $this->input->post('a2')
			);
			if ($aksi == 'tambah') {
				$proses = $this->M_model->tambah($data);
			}else if($aksi =='update'){
				$where = array('fc_jabatan' => $this->input->post('kode'));
