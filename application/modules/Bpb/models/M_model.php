...
// ---------------------------------------------------------------------
    ...
    {   
        $query = $this->db->query("select ifnull(c.fc_id,a.fc_id) as fc_id,a.fc_stock,a.fv_stock,a.variant,a.fv_satuan,concat('Rp.',IFNULL(c.fn_price,a.price)) as fn_price,a.fn_qty,ifnull(c.fn_qty,0) as fn_terima,concat('Rp.',IFNULL(c.fn_total,a.total)) as fn_total,a.fv_ket from v_detailPO a LEFT OUTER JOIN tm_bpb b ON b.fc_nopo=a.fc_nopo LEFT OUTER JOIN td_bpb c ON c.fc_nobpb=b.fc_nobpb and c.fc_stock=a.fc_stock WHERE a.fc_nopo='".$nota."'");
        return $query->num_rows();  
    } 
    ...
    function getdetailPO($kode){
    	$query = $this->db->query("select ifnull(c.fc_id,a.fc_id) as fc_id,a.fc_stock,a.fv_stock,a.variant,a.fv_satuan,concat('Rp.',IFNULL(c.fn_price,a.price)) as fn_price,a.fn_qty,ifnull(c.fn_qty,0) as fn_terima,concat('Rp.',IFNULL(c.fn_total,a.total)) as fn_total,a.fv_ket,a.fc_variant,a.fc_satuan from v_detailPO a LEFT OUTER JOIN tm_bpb b ON b.fc_nopo=a.fc_nopo LEFT OUTER JOIN td_bpb c ON c.fc_nobpb=b.fc_nobpb and c.fc_stock=a.fc_stock WHERE a.fc_id='".$kode."'");
    	return $query->row();
    }
}